...
$html_footer = <<<EOF
<div class="footer">
  <h6><a href="http://www.u-le.net" target="_blank">悠乐CMS内容管理系统</a></h6>
</div>
EOF;
...
switch ($_GET['step']) {
	...
	case 3:
		$install_error = '';
		$install_recover = '';
		// $demo_data =  file_exists('./data/think.sql') ? true : false;//测试数据库
...
function step3(&$install_error,&$install_recover){
	...
	$sql = file_get_contents("data/sto.sql");//创建数据库，如果有多个表可以按照上面的方式加入
	//$sql .= "\r\n".file_get_contents("data/haiqiang.sql");
	...
	/*生成系统index.php 配置文件*/
		...
		//定义资源文件路径
		define('STATIC_PATH','http://$httphost/'.ULE.'/Public');

		//定义公共路径
		define('COMMON_JS_PATH',STATIC_PATH.'/common/js/');
		define('COMMON_CSS_PATH',STATIC_PATH.'/common/css/');
		define('COMMON_IMG_PATH',STATIC_PATH.'/common/img/');


		//定义后台css路径
		define('ADMIN_PATH',STATIC_PATH.'/Admin/');
		define('ADMIN_CSS_PATH',STATIC_PATH.'/Admin/css/');
		define('ADMIN_JS_PATH',STATIC_PATH.'/Admin/js/');
		define('ADMIN_IMG_PATH',STATIC_PATH.'/Admin/img/');

		//定义layer路径
		define('ADMIN_LAYER_PATH',STATIC_PATH.'/layer/');
...
EOT;
	...
	$db->query("UPDATE `{$db_prefix}admin` SET `user` = '{$username}' ,`password` = '{$password}' WHERE `id`=1;");
	

	/*写入网站名称*/
	$db->query("UPDATE `{$db_prefix}system` SET `name` = '{$sitename}' WHERE `id`=1;");

	
	//如果需要陷入模拟数据的话 下面就是修改的模拟数据
    // if ($_POST['demo_data'] == '1'){
